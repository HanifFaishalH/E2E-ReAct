...
test.describe('Fitur Umpan Balik - Dosen', () => {
  const base = 'https://reportaction.dbsnetwork.my.id';
  const loginUrl = `${base}/login`;

  test('Dosen dapat memberikan umpan balik seperti alur pengguna asli', async ({ page }) => {
    test.setTimeout(120000);
    await page.setViewportSize({ width: 1920, height: 1080 });

    // 🔹 1. Login sebagai dosen
    await page.goto(loginUrl, { waitUntil: 'domcontentloaded' });
    await page.fill('#username', 'dosen');
    await page.fill('#password', 'dosen');
    ...
    // 🔹 7. Isi komentar
    const komentar = `Feedback otomatis dosen rating ${rating} - ${new Date().toLocaleTimeString()}`;
    ...
    // 🔹 9. Tunggu kirim selesai
    await page.waitForTimeout(2000);

    // 🔹 10. Screenshot bukti hasil
    await page.screenshot({
      path: 'test-results/dosen-feedback-finish.png',
      fullPage: true,
    });
  });
});
